...
breast.data <- Read10X(data.dir = "../data/cancer/breast/10x/")
breast <- CreateSeuratObject(counts = breast.data, project = "breast_GSE110686", min.cells = 3, min.features = 200)
breast
## An object of class Seurat 
## 15623 features across 6302 samples within 1 assay 
## Active assay: RNA (15623 features, 0 variable features)
breast$type <- "breast"

#read COlorectal Data

colo.data <- read.csv(file = "../data/cancer/colorectal/GSE108989_TPM.txt", sep = "\t", header = TRUE, row.names = NULL)
# colo.data <- subset(colo.data, select = -c(geneID) )
names <- make.unique(colo.data$symbol)
.rowNamesDF(colo.data, make.names = TRUE) <- names
# rownames(colo.data) <- names
colo.data <- colo.data[, -1]  # get rid of old names
colo <- CreateSeuratObject(counts = colo.data, project = "colo_GSE75688TPM", min.cells = 3, min.features = 200)
colo
## An object of class Seurat 
## 22833 features across 11138 samples within 1 assay 
## Active assay: RNA (22833 features, 0 variable features)
colo$type <- "colo"


# Read liver data
liver.data <- read.csv(file = "../data/cancer/liver/GSE98638_TPM.txt", sep = "\t", header = TRUE, row.names = NULL)
# liver.data <- subset(liver.data, select = -c(geneID) )
names <- make.unique(liver.data$symbol)
.rowNamesDF(liver.data, make.names = TRUE) <- names
# rownames(liver.data) <- names
liver.data <- liver.data[, -1]  # get rid of old names
liver <- CreateSeuratObject(counts = liver.data, project = "liver_GSE98638", min.cells = 3, min.features = 200)
liver
liver$type <- "liver"

#Read Lung data

lung.data <- read.csv(file = "../data/cancer/lung/GSE99254_TPM.txt", sep = "\t", header = TRUE, row.names = NULL)
# lung.data <- subset(lung.data, select = -c(geneID) )
names <- make.unique(lung.data$symbol)
.rowNamesDF(lung.data, make.names = TRUE) <- names

# rownames(lung.data) <- names
lung.data <- lung.data[, -1]  # get rid of old names
lung <- CreateSeuratObject(counts = lung.data, project = "lung_GSE99254", min.cells = 3, min.features = 200)
lung

#read Pancreatic data
panc.data <- read.csv(file = "../data/cancer/pancreatic/CRA001160.txt", sep = " ", header = TRUE, row.names = NULL)
names <- make.unique(panc.data$row.names)
.rowNamesDF(panc.data, make.names = TRUE) <- names

# rownames(panc.data) <- names
panc.data <- panc.data[, -1]  # get rid of old names
panc <- CreateSeuratObject(counts = panc.data, project = "panc_GSE98638", min.cells = 3, min.features = 200)
panc
## An object of class Seurat 
## 23376 features across 57530 samples within 1 assay 
## Active assay: RNA (23376 features, 0 variable features)
# panc$type<-'panc'
panc$type <- readLines(con = "../data/cancer/pancreatic/celltype.txt")
tcell <- subset(x = panc, subset = type == "T cell")
tcell$type = "panc"
panc <- tcell

#Merging populations
both <- merge(x = breast, y = c(colo, liver, lung, panc), project = "tcell")


#Normalizing data and PCA analysis


both <- NormalizeData(object = both)
both <- FindVariableFeatures(object = both)
both <- ScaleData(object = both)
both <- RunPCA(object = both)
both <- FindNeighbors(object = both)
both <- FindClusters(object = both)
both <- RunTSNE(object = both, check_duplicates = FALSE)
both <- RunUMAP(both, dims = 1:10, reduction.name = "UMAP")
